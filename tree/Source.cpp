...
tree* tree::rotateRight(tree* y) 
{
	...
	// Обновляем высоту узлов после поворота
...
tree* tree::insertBT(tree* tr, int data)
{
	...
	// Иначе рекурсивно вставляем новый узел в соответствующую подветку
	if (data < tr->data)
		tr->left = insertBT(tr->left, data);
	else if (data > tr->data)
		tr->right = insertBT(tr->right, data);
	else
		return tr; // Дублирующийся ключ не допускается

	// Обновляем высоту текущего узла
	tr->height = max(getHeight(tr->left), getHeight(tr->right)) + 1;

	// Выполняем повороты, если дерево стало несбалансированным
...
public:
	TreePrint() 
	{
		window.create(sf::VideoMode(1200, 700), L"Дерево");
	}

	void drawNode(tree* node, int x, int y, int level)
	{
		const Color VertexDefaultColor = Color(145, 151, 235);
		if (node == nullptr)
			return;
	...
	{
		const Color WindowColor = Color(200, 200, 200);
